...
namespace ExtraFunctions
{
    public static class DebugToolsScenarioTools
    {
                                ...
                                if (plantDefs != null)
                                {
                                    DebugToolsGeneral.GenericRectTool("Generate plants", delegate (CellRect rect)
                                    {
                                        foreach (IntVec3 cell in rect.Cells)
                                        {
                                            if (Rand.Chance(j / 10f))
                                            {
                                                ThingDef plant = biomeDef.wildPlants.Where(x => x.plant?.plant?.pollution != Pollution.PollutedOnly).RandomElementByWeightWithDefault(x => x.commonality, 0).plant;
                                                GenSpawn.Spawn(plant, cell, Find.CurrentMap);
                                            }
                                        }
                                    });
                        ...
                        foreach (IntVec3 cell in rect.Cells)
                        {
                            foreach (Thing thing in cell.GetThingList(Find.CurrentMap))
                            {
                                if (thing.def.MadeFromStuff)
                                {
                                    ...
                                    StatDefOf.MaxHitPoints.Worker.immutableStatCache?.Remove(thing, out var _);
                                    StatDefOf.MaxHitPoints.Worker.temporaryStatCache?.Remove(thing, out var _);
        ...
        private static void MakeFog()
        {
            DebugToolsGeneral.GenericRectTool("Make fog", delegate (CellRect rect)
            {
                ...
                foreach (IntVec3 cell in rect.Cells)
                {
                    ...
                    {
                        Find.CurrentMap.fogGrid.fogGrid.Set(cellIndices.CellToIndex(cell), true);
        ...
        [DebugAction("Scenario Tools", "Set faction (rect)", false, false, false, false, false, 0, actionType = DebugActionType.Action, allowedGameStates = AllowedGameStates.PlayingOnMap, displayPriority = 100)]
        ...
        [DebugAction("Scenario Tools", "Make gears biocoded", false, false, false, false, false, 0, actionType = DebugActionType.ToolMap, allowedGameStates = AllowedGameStates.PlayingOnMap)]
        ...
        [DebugAction("Scenario Tools", "Make gears biocoded (rect)", false, false, false, false, false, 0, actionType = DebugActionType.Action, allowedGameStates = AllowedGameStates.PlayingOnMap)]
